...
use Bowhead\Models\BhConfigs;
use Bowhead\Models\BhPopularExchanges;
...
class SetupController extends Controller
{
    ...
    {
        $input = $request->all();
        $preferred_exchanges = BhPopularExchanges::where('exch_id', $input['ex'])->get()->first();
    ...
    public function setup2(Request $request)
    {
        ...
        $preferred_exchanges = BhExchanges::whereIn('exchange', $_preferred_exchanges)->get();
        foreach($preferred_exchanges as $pe) {
            $preferred[] = $pe->id;
        }//*/
        $vars = $exhange_links = [];
        $_exhange_links = BhExchanges::get();
        ...
        if ($input['coinigy']) {
            ...
            $preferred_exchanges = BhPopularExchanges::where('coinigy', 1)->get();
            foreach($preferred_exchanges as $pe) {
            ...
            BhConfigs::updateOrCreate(['item' => 'COINIGY_SEC'], ['value' => $input['apisecret']]);
            ...
            foreach($accounts as $acct) {
                ...
                $exhange_model = BhExchanges::where('coinigy','=', 1)->where('exchange','=', $acct['exch_name'])->get()->first();
                if ($exhange_model) {
            ...
            foreach ($response['data'] as $exch) {
                $exhange_model = BhExchanges::where('coinigy','=', 1)->where('exchange','=', $exch['exch_name'])->get()->first();
        ...
        /** CCXT */
            ...
            BhConfigs::updateOrCreate(['item' => 'COINIGY'], ['value' => 0]);
            $exchanges = $this->get_exchanges(); // via trait
            $all_ccxt = BhExchanges::whereIn('exchange', $exchanges)->get();
    ...
    public function setup3(Request $request)
    {
        ...
        BhConfigs::updateOrCreate(['item' => 'EXCHANGES'], ['value' => join(',', $selected)]);
    ...
    public function setup4(Request $request)
    {
        ...
        BhConfigs::updateOrCreate(['item' => 'PAIRS'], ['value' => join(',', $selected)]);

        // why is postgres having an issue with this?
        #BhConfigs::updateOrCreate(['item' => 'SETUP'], ['value' => 1]);
    ...
    public function exchanges(Request $request)
    {
        ...
        foreach($input as $key => $inp) {
            if ($key[0] <> '_'){
                BhConfigs::updateOrCreate(['item' => $key], ['value' => $inp]);
            }
        }

        $config_names = [];
        $exchange_list = explode(',', Traits\Config::bowhead_config('EXCHANGES'));
        $exchanges = BhExchanges::whereIn('id', $exchange_list)->get();
        ...
        $preferred_exchanges = BhPopularExchanges::get();
        foreach($preferred_exchanges as $pe) {
            $preferred[$pe->exch_id] = $pe->link;
        }
        $vars['notice'] = '';
